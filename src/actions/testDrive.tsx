...
interface UserTestDrivesResult {
  success: boolean;
...
export interface FormattedBooking {
  ...
  userId: string;
  carId: string;
  car: ReturnType<typeof serializeCarData> | null;
  bookingDate: string;
  startTime: string;
  endTime: string;
 ...
 */
export async function getUserTestDrives(): Promise<UserTestDrivesResult> {
  try {
    const { userId } = await auth();
    if (!userId) {
      return {
    ...
    const formattedBookings = bookings.map((booking) => ({
      id: booking.id,
      userId: booking.userId, // <-- Add this line
      carId: booking.carId,
      car: serializeCarData(booking.car as Car, false),
 ...
 */
export async function cancelTestDrive(bookingId: string): Promise<CancelBookingResult> {
  try {
    const { userId } = await auth();
    if (!userId) {
      return {
