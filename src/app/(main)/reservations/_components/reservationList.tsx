...
import { cancelTestDrive, FormattedBooking } from "@/actions/testDrive";
...
export function ReservationsList({ initialData }: ReservationsListProps) {
  ...
  const upcomingBookings = initialData?.data?.filter((booking: FormattedBooking) =>
    ["PENDING", "CONFIRMED"].includes(booking.status)
  );

  const pastBookings = initialData?.data?.filter((booking: FormattedBooking) =>
